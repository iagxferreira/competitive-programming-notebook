...
leetcode | Medium | tree, depth-first-search, binary-search-tree
...
`T` that has both `p` and `q` as descendants (where we allow a node to
be a descendant of itself).”

The number of nodes in the tree is in the range `[2, 10^5]`.   `-10^9 <=
Node.val <= 10^9`   `p != q`   `p` and `q` will exist in the BST.
